...
- 起動時、未保存日がなければ現在日のGitHubデータを自動pull
- スマホでは画面上端から下へ引っ張って現在日をpull
...
- 起動時、未保存日が1件もなくTokenが設定済みなら現在日をGitHubから自動pull
- スマホでは画面上端から一定距離下へ引っ張って離すと現在日をpull
...
実行中タスクの `終了見込み変更` は、終了見込み時刻そのものを独立データとして保存しません。開始時刻との差分を見積時間へ換算し、Markdown上は `(200m)` のような見積分だけを保存します。画面上の終了見込みは `開始時刻 + 見積時間` から都度計算します。

旧形式の `[ETA HH:MM]` は互換入力として読み込み時に除去し、次回保存では出力しません。
