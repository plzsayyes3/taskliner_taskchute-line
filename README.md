...
GitHub同期を使う場合は、画面右下の `⚙` を開き、`GitHub同期` から日別MarkdownをGitHubへ保存できます。また、通常画面上部の `保存` ボタンから、現在日の未保存変更をすぐGitHubへ保存できます。
...
- 上部の `保存` ボタンを押すと、未保存の現在日をすぐGitHubへ保存
- すでに `保存済み` の場合、上部の `保存` ボタンは不要なGitHub更新を行わない
...
自動保存は競合時に無断で上書きしません。GitHub側に前回確認後の変更がある場合や、まだ基準SHAを持っていない既存ファイルの場合は `競合あり` とし、`GitHubから読込` または手動の `GitHubへ保存` で確認してから処理します。
...
5. `接続確認` を押す
6. 初回または基準SHAがない既存日では `GitHubから読込` を行う
7. 以後は通常操作後の自動保存、上部の `保存`、または必要に応じて設定内の `GitHubへ保存` を使う
